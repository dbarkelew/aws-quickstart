...
# If using us-east-1 then the LocationConstraint fails and needs to be removed since us-east-1 is the default.
if [ "$?" -eq 255 ]; then
...
aws s3 sync --acl=public-read ./bin "s3://${S3_BUCKET}/${S3_PREFIX}bin/"
